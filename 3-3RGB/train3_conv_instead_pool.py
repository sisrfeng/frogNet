...
parser.add_argument('--dist_tolerance', type=int, default=4, help='defalut: 4')
...
myXPU =torch.device('cuda')   # ~/dotF/dl_func.py
...
def seed_worker(worker_id):
    #DataLoader will reseed workers following Randomness in multi-process data loading algorithm.不过这里好像不涉及。
...
def train(epoch):
    ...
    for batch_idx, (a_batch_x, y_true) in enumerate(train_loader):
        ...
        bar = 100.0 * (batch_idx+1) * len(a_batch_x) / len(train_loader.dataset)
        if int(bar+1) % 10==0:
            print('Epoch" {} [{}/{} ({:.0f}%) '.format(
                    epoch, (batch_idx+1) * len(a_batch_x), len(train_loader.dataset), bar )
                )
